...
If an $n$-sided **polygon** with equal side lengths is inscribed in a **unit circle** and the number of sides increase ($n$ becomes larger), the perimeter of the polygon will converge to the circumference of the unit circle $2 \pi$. Seen in the image below, the the first polygon is a triangle, the second a square and the last is a shape that approximates a circle. If $n \to \infty$ then the perimeter goes to $2\pi$.
### Concepts
>This problem illustrates a connection between ideas from **calculus** and **geometry**.
"""

# ╔═╡ b22681d4-1473-46a5-ad29-77578542c67b
md"""## Computing Side Length"""

# ╔═╡ 0ad98f8f-fa71-4d02-aad8-21da1ae99375
md"""
Before computing the perimeter of an $n$ sided polygon inscribed in the unit circle, the task is to compute the length of just one of the $n$ sides. The components needed for this computation are points in the $x-y$ plane along with the Pythagorean theorem for computing distance between two points in the plane. 

A triangle is described by three points  $A$, $B$, and $C$.
Where the points are represented as pairs of numbers in the $x-y$
plane. The first number in the pair corresponds to the $x$ position and the second the $y$ position.

$A = (A_x, A_y),~ B = (B_x, B_y),~ C = (C_x, C_y)$

With the line connecting $B$ and $C$ one side of of the $n$ sided polygon. The length of the line can be computed with the Pythagorean theorem. 

$\overline{BC} = \sqrt{(C_x - B_x)^2 + (C_y - B_y)^2}$


Point $A$ is at the origin and has coordinates $(0, 0)$, and points $B$ and $C$ have coordinates $(1, 0)$, $(\cos(2 \pi / n),~ \sin(2 \pi / n))$ respectively.

Substiting these values into the formula for $\overline{BC}$,

$\overline{BC} = \sqrt{(\cos(2 \pi / n) - 1)^2 + (\sin(2 \pi / n))^2}$
Expandind the squared terms

$\overline{BC} = \sqrt{1^2 -2\cos(2 \pi / n) + \cos(2 \pi / n)^2  + \sin(2 \pi / n)^2}$

Simplifying the expression for the length

$\overline{BC} = \sqrt{2 - 2\cos(2 \pi / n)}$

The length of the n-sided polygon perimeter is

$\boxed{n\sqrt{2 - 2\cos(2 \pi / n)}}$


"""

# ╔═╡ 88020167-1fd2-4174-93cf-9d970be116f8
md"N-Sides $(@bind n Slider(3:100))"

# ╔═╡ 6bbd6510-6d8d-47eb-8ff8-3219cb4af9a4
md"""
## Proof
Now that some intuition has been developed, I will prove that as $n$ gets infinitely large the perimeter of an equilateral $n$-sided polygon inscribed in a unit circle converges to $2\pi$. The first step is to state a formula for measuring the length of one of the $n$ sides of a given polygon. This was done in the **Computing Side Length** section and is expression $(1)$.

$\tag{1} \sqrt{2 - 2\cos{(2\pi / n)}}$

The total perimeter is just expression $(1)$ times $n$, which is seen in expression $(2)$.

$\tag{2} n\sqrt{2 - 2\cos{(2\pi / n)}}$

Since the goal is to find the perimeter as $n \to \infty$ the limit of expression $(2)$ needs to be solved.

$\tag{3} \lim_{n \to \infty} n\sqrt{2 - 2\cos{(2\pi / n)}}$

However it is not obvious how this limit expression $(3)$ can be reduced to find the desired answer of $2 \pi$. To facilitate this goal, $\cos{(2\pi / n)}$ will be approximated quadratically near $0$, since as $n \to \infty$, $(2\pi / n \to 0)$.

$\tag{4} \cos{(2\pi / n)} \approx 1 - \frac{2\pi^2}{n^2}$

Substituting this quadratic approximation for $\cos{(2\pi / n)}$ back into the limit expression $(3)$ and simplifying forms a new expression $(5)$, and is now in a form that can be easily reduced.

$\tag{5} \boxed{  \lim_{n \to \infty}  n\sqrt{\frac{4\pi^2}{n^2}}= \lim_{n \to \infty} 2\pi = 2\pi}$
...
# ╔═╡ 5d3ed5fa-11f8-4523-a142-a0ec75182558
selectedColorSet = colors

# ╔═╡ f90bdeba-de50-420b-9474-7bba1062b29b
colors
...
function drawInEachGridSquare(nrows, ncols, width, height, object)
	...
	for i in 1:nrows
		for j in 1:ncols
			...
			move(p)
			object(count, 30, p.x, p.y,selectedColorSet[count - 2])
			circle(p, 30, :stroke)
			count += 1
		end
	end
...
function drawPolygon(nSides, r, x, y, color)
	...
	for i in 2:nSides
	line(Point(x + r*cos(i*(2*pi / nSides)), y + -r*sin(i*(2*pi / nSides))))
	end
	closepath()
...
function drawGrid(nrows, ncols, width, height)
	...
	for i in 0:nrows
		...
		line(Point(width, i* (height/nrows)))
	end
	# Draw cols
	for i in 0:ncols
		move(Point(i*(width/ncols),0))
		line(Point(i*(width/ncols), height))
	end
	closepath()
	strokepath()
end
...
drawShape(translateY(
	translateX(reflectX(triangle₁), 80)
	,120))
sethue("darkred")
...
manifest_format = "2.0"
project_hash = "607cfcb43f8a0e3240f3912d62649615c90ab13c"
